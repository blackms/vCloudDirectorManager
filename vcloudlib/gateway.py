...
from schema.vcd.v1_5.schemas.vcloud.networkType import NatRuleType, GatewayNatRuleType, ReferenceType, NatServiceType, \
    FirewallRuleType, ProtocolsType, DhcpPoolServiceType, GatewayIpsecVpnServiceType, GatewayIpsecVpnEndpointType, \
    GatewayIpsecVpnTunnelType, IpsecVpnSubnetType, IpsecVpnThirdPartyPeerType, DhcpServiceType, GatewayDhcpServiceType
from iptools import IpRange
from helper import CommonUtils
from xml.etree import ElementTree as ET
from netaddr import *
from vcloudlib.helper import _get_logger, Http
...
class Gateway(object):
    ...
    def get_interfaces(self, interface_type):
        ...
        gatewayInterfaces = filter(lambda gatewayInterface: gatewayInterface.get_InterfaceType() == interface_type,
    ...
    def get_full_public_ips(self):
        result = []
        for gatewayInterface in filter(lambda x: 'External Network' in x.Name, self.get_interfaces('uplink')):
            for subnetParticipation in gatewayInterface.get_SubnetParticipation():
                result.append(subnetParticipation.IpAddress)
                ipRanges = subnetParticipation.get_IpRanges()
                if ipRanges:
                    for ipRange in ipRanges.get_IpRange():
                        startAddress = ipRange.get_StartAddress()
    ...
    def get_nat_rules(self):
        ...
        natServiceList = filter(lambda service: service.__class__.__name__ == "NatServiceType",
    ...
    def add_nat_rule(self, rule_type, original_ip, original_port, translated_ip, translated_port, protocol):
        ...
        natService = None
        edgeGatewayServiceConfiguration = self.me.get_Configuration().get_EdgeGatewayServiceConfiguration()
        natServices = filter(lambda service: service.__class__.__name__ == "NatServiceType",
                             edgeGatewayServiceConfiguration.get_NetworkService())
    ...
    def is_fw_enabled(self):
        edgeGatewayServiceConfiguration = self.me.get_Configuration().get_EdgeGatewayServiceConfiguration()
        services = filter(lambda service: service.__class__.__name__ == "FirewallServiceType",
                          edgeGatewayServiceConfiguration.get_NetworkService())
        return len(services) == 1 and services[0].get_IsEnabled()

    def is_dhcp_enabled(self):
        edgeGatewayServiceConfiguration = self.me.get_Configuration().get_EdgeGatewayServiceConfiguration()
        services = filter(lambda service: service.__class__.__name__ == "GatewayDhcpServiceType",
                          edgeGatewayServiceConfiguration.get_NetworkService())
        return len(services) == 1 and services[0].get_IsEnabled()

    def is_nat_enabled(self):
        edgeGatewayServiceConfiguration = self.me.get_Configuration().get_EdgeGatewayServiceConfiguration()
        services = filter(lambda service: service.__class__.__name__ == "NatServiceType",
                          edgeGatewayServiceConfiguration.get_NetworkService())
        return len(services) == 1 and services[0].get_IsEnabled()
    ...
    def is_vpn_enabled(self):
        edgeGatewayServiceConfiguration = self.me.get_Configuration().get_EdgeGatewayServiceConfiguration()
        services = filter(lambda service: service.__class__.__name__ == "GatewayIpsecVpnServiceType",
                          edgeGatewayServiceConfiguration.get_NetworkService())
        return len(services) == 1 and services[0].get_IsEnabled()
    ...
    def set_syslog_conf(self, syslog_server_ip):
        headers = self.headers
        headers['Content-Type'] = "application/vnd.vmware.vcloud.SyslogSettings+xml"
        body = """
        ...
        """.format(syslog_server_ip)
        geLink = '{}/syslog/config'.format(self.me.href)
        self.response = Http.post(geLink,
                                  data=body,
                                  headers=headers,
                                  verify=self.verify,
                                  logger=self.logger)
